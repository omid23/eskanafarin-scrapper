...
def all_posts(user_name, password, scrapping_pages=10):
    try:
        page_number = 1
        ...
        while page_number <= scrapping_pages:
            ...
            tokens += [hrefs[item]['href'].split("?")[1].split("&")[0].split("bcode=")[1] for item in range(len(items))]
            page_number += 1
...
def single_post_detail(token, authentication):
    mortgage_fee = None
    rent_fee = None
    price_fee = None
